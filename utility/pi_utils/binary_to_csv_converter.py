...
def save_to_research_csv(points, filename):
    df = pd.DataFrame(points, columns=['FrameNumber', 'ObjectNumber', 'Range', 'Velocity', 'PeakValue', 'x', 'y', 'z'])
...
def main(data_fn, cfg_dir):
    ...
    # Initialize v1 processor (removed the angle_bin arguments as v1 doesn't use them)
    mmw_proc = CubeProcessor(setting)
    ...
    with open(data_fn, "rb") as file:
        while True:
            ...
            if prev_rdm is not None:
                # 1. Background subtraction on the 2D Map
                processed_rdm = np.abs(current_rdm - prev_rdm)
                ...
                # 4. LOG SCALING
                processed_rdm = np.log10(processed_rdm + 1e-9)
            else:
                processed_rdm = np.zeros_like(current_rdm)
                
            prev_rdm = current_rdm      

            # 5. Extract Top 20 Peaks from the 2D Map
            flat_indices = np.argsort(processed_rdm.flatten())[-20:]
            # FIXED AXIS BUG: Unravel correctly into Doppler (0) and Range (1)
            ...
            for i in range(len(r_idx)):
                ...
                # Convert to Physical Units (Matching inference script multipliers)
                r_val = r * res_range
                v_val = (d - 30) * res_doppler * 1.5 
                ...
                # 7. 3D Spherical to Cartesian Conversion
                x = r_val * np.sin(theta) * np.cos(phi) # Horizontal (Left/Right)
                y = r_val * np.cos(theta) * np.cos(phi) # Depth (Push/Pull)
                z = r_val * np.sin(phi)                 # Vertical (Up/Down)

    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Inference runs if buffer is full, cooldown is 0, AND someone is actively moving
                            ...
                            if conf.item() >= req_conf and len(self.prediction_history) >= req_frames:
                                ...
                                if all(g == gesture_name for g in recent_streak):
                                    ...
                                    if self.os_control_active:
                                        # Execute in background to prevent loop blocking/temporal stretching
                                    ...
                                    # THE KILL SHOT: Hard-pad the array with Zero-Doppler silence.
                                    # Prevents hybrid buffers on cooldown exit.
                                    self.r_buf = [0.0] * 40
                                    self.v_buf = [16.0] * 40  # 16.0 is the exact Zero-Doppler index
                                    self.a_buf = [0.0] * 40
                                    self.e_buf = [0.0] * 40
                        else:
                            self.prediction_history.clear()
